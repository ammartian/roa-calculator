...
import type { Funnel2Computation } from "@/lib/funnelBudgetCalculations";
    ...
    return (
        <Card className="border-primary/20 bg-primary/5">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                    <OutputRow
                        label={t.totalRegFeeCollected}
                        value={
                            isValid
                                ? formatRinggit(results.totalRegFeeCollected)
                                : "—"
                        }
                    />
                    <OutputRow
                        label={t.leadsRequired}
                        value={formatQuantity(results.leadsRequired, isValid)}
                    />
                    <OutputRow
                        label={t.potentialActiveAgents}
                        value={formatQuantity(
                            results.potentialActiveAgents,
                            isValid
                        )}
                    />
                    <OutputRow
                    ...
                    <OutputRow
                        label={t.maxCPL}
                        value={isValid ? formatRinggit(results.maxCPL) : "—"}
                    />
                    <OutputRow
                    ...
                    <OutputRow
                        label={t.roasPerYear}
                        value={
                            isValid && results.roasPerYear > 0
                                ? results.roasPerYear.toFixed(2)
                                : "—"
                        }
                    />
                </div>
